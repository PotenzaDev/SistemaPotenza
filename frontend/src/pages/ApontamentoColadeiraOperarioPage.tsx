...
import { biparColadeira, biparFichaColadeira } from '@/api/apontamentoColadeira'
...
import { formatDuracao, derivarFase, apiMsg, mensagemFinalizarTurno, horarioLiberacaoTurno, fmtHoraDate } from '@/lib/apontamentoFormat'

/** Metros lineares de borda formatados com 2 casas — ex.: "46,00 m". */
function fmtMetros(valor: number): string {
  return `${valor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} m`
}

function totalMetrosLineares(fichas: FichaApontamento[]): number {
  return fichas.reduce((sum, f) => sum + (f.metros_lineares_borda ?? 0), 0)
}

/**
 * Tela de apontamento da Coladeira — mesmo fluxo (setup → bipar ficha →
 * finalizar) da tela genérica (ApontamentoOperarioPage), mas o indicador de
 * progresso é metros lineares de fita de borda (acumulado a partir de
 * fichas[].metros_lineares_borda, calculado no backend) em vez de contagem
 * de peças. Não oferece "ficha de setup" (cabeçote/broca, exclusivo da
 * furadeira) nem múltiplos apontamentos simultâneos/segunda passagem — fora
 * do escopo pedido para este fluxo.
 */
export function ApontamentoColadeiraOperarioPage() {
  ...
  const fase = !apontamento
    ? 'aguardando'
    : finalizando
      ? 'finalizando'
      : derivarFase(apontamento)
  ...
  const totalMetros  = useMemo(() => totalMetrosLineares(apontamento?.fichas ?? []), [apontamento])
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (!s.maquina.etapa_fluxo?.calcula_metragem_borda) { navigate('/operario/apontamento', { replace: true }); return }
      setSessao(s)
      ...
      const focoInicial = derivarFase(ap)
      if (focoInicial === 'em_pausa_setup' || focoInicial === 'em_pausa_producao') {
        const openPausa = ap.pausas.find(p => p.fim === null)
        if (openPausa?.is_sistema) setSaiuSemPausar(true)
      }

      if (focoInicial === 'em_producao' || focoInicial === 'em_pausa_producao') {
        const init: Record<number, string> = {}
        ap.fichas.forEach(fi => { init[fi.id] = String(fi.qtd_peca) })
        setQtdsFichas(init)
      }
  ...
  useEffect(() => {
    if (!apontamento || (apontamento.status !== 'em_setup' && apontamento.status !== 'em_producao')) return
  ...
  async function executarBiparFicha(confirmar: boolean) {
    ...
    const ap = await biparFichaColadeira(apontamento.id, {
      cod_peca:    parsedBarcode.cod_peca,
      ordem_lote:  parsedBarcode.ordem_lote,
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      {fase === 'em_producao' && apontamento && (
        <>
          {/* Card de info: timer + metros lineares acumulados (indicador principal da Coladeira) */}
          <div className="bg-[#0f1923] border border-white/5 rounded-xl overflow-hidden">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    {apontamento.qtde_total !== null && (
                      <span className="ml-2 text-slate-400">{totalBipado}/{apontamento.qtde_total} pç</span>
                    )}
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 px-5 py-3">
              <Ruler className="w-4 h-4 text-[#00aa84] shrink-0" />
              <p className="text-xs text-slate-500">Metros de borda colada</p>
              <p className="ml-auto text-lg font-mono font-bold tabular-nums text-[#00aa84]">{fmtMetros(totalMetros)}</p>
            </div>
          </div>
          ...
          {/* Fichas — mostra metros lineares de cada ficha (não a tela genérica de peças/pilha) */}
          {apontamento.fichas.length > 0 && (
            <div className="bg-[#0f1923] border border-white/5 rounded-xl overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-3 border-b border-white/5">
                <Ruler className="w-4 h-4 text-slate-400" />
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
                  Fichas deste lote ({apontamento.fichas.length})
                </p>
              </div>
                  ...
                  <div key={f.id} className="flex items-center justify-between gap-3 px-5 py-3">
                    <div>
                      ...
                      <span className="ml-2 text-xs text-slate-500">
                        pilha {f.pilha}{f.total_pilhas > 0 ? ` / ${f.total_pilhas}` : ''} · {f.qtd_peca} pç
                      </span>
                    </div>
                    <div className="bg-[#00aa84]/10 border border-[#00aa84]/20 rounded-lg px-3 py-1.5 text-center min-w-[80px]">
                      <p className="text-xs text-slate-500 leading-none mb-0.5">Metros</p>
                      <p className="text-sm font-bold text-[#00aa84] tabular-nums">
                        {fmtMetros(f.metros_lineares_borda ?? 0)}
                      </p>
                    </div>
                  </div>
                ))}
      ...
      {fase === 'finalizando' && apontamento && (
        <div className="bg-[#0f1923] border border-white/5 rounded-xl overflow-hidden">
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="flex items-center justify-between bg-white/[0.03] rounded-lg px-3 py-2.5">
              <p className="text-xs text-slate-500">Metros de borda colada</p>
              <p className="text-sm font-mono font-semibold text-[#00aa84]">{fmtMetros(totalMetros)}</p>
            </div>
            <div className="space-y-3">
              {apontamento.fichas.map(f => (
                <div key={f.id} className="bg-white/[0.03] border border-white/5 rounded-lg px-4 py-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-xs font-mono font-semibold text-white">{f.cod_peca}</span>
                      <span className="ml-2 text-xs text-slate-500">
                        pilha {f.pilha}{f.total_pilhas > 0 ? ` / ${f.total_pilhas}` : ''}
                      </span>
                    </div>
                    <span className="text-xs text-slate-500">{fmtMetros(f.metros_lineares_borda ?? 0)}</span>
                  </div>
      ...
      {fase === 'concluido' && apontamento && (
        <div className="bg-[#0f1923] border border-white/5 rounded-xl overflow-hidden">
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <InfoCard label="Metros de borda colada" value={fmtMetros(totalMetros)} highlight />
            {apontamento.fichas.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="divide-y divide-white/5 bg-white/[0.02] rounded-lg overflow-hidden">
                  <div className="grid grid-cols-3 px-4 py-1.5">
                    ...
                    <span className="text-[10px] font-semibold text-slate-600 uppercase tracking-wider text-right">Metros</span>
                  </div>
                  {apontamento.fichas.map(f => (
